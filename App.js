...
const App = () => {
  ...
  const getResult = async (path, response) => {
    ...
    if (res?.data?.class) {
      ...
      let confidence = parseFloat(res.data.confidence) * 100;
      ...
      {(result && label && (
        ...
        (image && <Text style={styles.emptyText}>{label}</Text>) || (
          <Text style={styles.emptyText}>
            Use below buttons to select a picture of a wheat plant leaf.
          </Text>
        )}
...
const styles = StyleSheet.create({
  ...
  labelText: {color: '#000', fontSize: 20, ...fonts.Bold},
